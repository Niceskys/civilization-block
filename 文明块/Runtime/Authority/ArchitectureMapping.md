...
`GameTimeDifficulty.cs`定义权威Tick、暂停/倍速换算、昼夜阶段纯派生规则及结构事故难度策略。`DayNightCycle`只根据`SimulationTick`派生白天/夜晚，不保存第二套昼夜状态，也不直接驱动生产、NPC、怪物或UI行为。难度配置进入`GameState`、存档和StateHash；服务器推进Tick，客户端墙钟不参与宽限或崩塌裁决。
